        ...
        <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{($tipx=='new')? url('/mkl/store/') : url('/mkl/update/'.$header->NO_ID ) }}" method="POST" name ="entri" id="entri" >
						...
						<div class="mt-3 col-md-12 form-group row">
							<div class="col-md-4">
								<button type="button" hidden id='TOPX'  onclick="location.href='{{url('/mkl/edit/?idx=' .$idx. '&tipx=top')}}'" class="btn btn-outline-primary">Top</button>
								<button type="button" hidden id='PREVX' onclick="location.href='{{url('/mkl/edit/?idx='.$header->NO_ID.'&tipx=prev&kodex='.$header->ACNO )}}'" class="btn btn-outline-primary">Prev</button>
								<button type="button" hidden id='NEXTX' onclick="location.href='{{url('/mkl/edit/?idx='.$header->NO_ID.'&tipx=next&kodex='.$header->ACNO )}}'" class="btn btn-outline-primary">Next</button>
								<button type="button" hidden id='BOTTOMX' onclick="location.href='{{url('/mkl/edit/?idx=' .$idx. '&tipx=bottom')}}'" class="btn btn-outline-primary">Bottom</button>
							</div>
							<div class="col-md-5">
							...
							<div class="col-md-3">
								...
								<button type="button" id='CLOSEX'  onclick="location.href='{{url('/mkl' )}}'" class="btn btn-outline-secondary">Close</button>
...
<script>
	...
	function hapusTrans() {
		...
		{
			window.location ="{{url('/mkl/delete/'.$header->NO_ID )}}'";
			//return true;
		} 
		return false;
	}
	...
	function CariBukti() {
		...
		var cari = $("#CARI").val();
		var loc = "{{ url('/mkl/edit/') }}" + '?idx={{ $header->NO_ID}}&tipx=search&kodex=' +encodeURIComponent(cari);
		window.location = loc;
