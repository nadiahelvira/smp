...
class RKartusoController extends Controller
{
    ...
    public function kartu()
    {
        ...
        return view('oreport_so.kartu')->with(['kodes' => $sup])->with(['hasil' => []]);
    }
	
	public function getSoKartu(Request $request)
    {
	...
	public function sisa()
    {
        ...
        return view('oreport_so.sisa')->with(['kodes' => $sup])->with(['hasil' => []]);
    }
	
	public function getSoSisa(Request $request)
    {
		$periode = $request->session()->get('periode')['bulan']. '/' . $request->session()->get('periode')['tahun'];
		$bulan = substr($periode,0,2);
		$tahun = substr($periode,3,4);
	...
	public function jasperSoSisaReport(Request $request) 
	{
		$file 	= 'sisah';
		...
		{
			return view('oreport_so.sisa')->with(['hasil' => $query]);
		}
	...
	public function jasperSoKartu(Request $request) 
	{
		...
		$queryakum = DB::SELECT("SET @akum:=0;");
		// $query = DB::SELECT("SELECT *,@akum:=@akum+TOTAL-BAYAR SALDO from
		// 	(
		...
		// 		SELECT NO_BUKTI, TGL, KODES, NAMAS, 0 AS TOTAL, BAYAR  
		// 		from po where PER='$periode' $sup
				
		// 	) as  kartuh;
		// ");

		$query = DB::SELECT("SELECT NO_PO AS NO_BUKTI, DATE_FORMAT(so.TGL,'%d/%m/%Y') as TGL, NAMAS, NA_BRG, KG, KIRIM, SISA, HARGA, 
									TOTAL, NOTES  
								from so where PER='$periode' $sup
							");
		...
		if($request->has('filter'))
		{
			return view('oreport_so.kartu')->with(['hasil' => $query]);
		}
