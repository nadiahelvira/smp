...
class RKartupoController extends Controller
{
    ...
    public function kartu()
    {
        ...
        return view('oreport_po.kartu')->with(['kodes' => $sup])->with(['hasil' => []]);
    }
	
	public function getPoKartu(Request $request)
    {
	...
	public function sisa()
    {
        ...
        return view('oreport_po.sisa')->with(['kodes' => $sup])->with(['hasil' => []]);
    }
	
	public function getPoSisa(Request $request)
    {
		$periode = $request->session()->get('periode')['bulan']. '/' . $request->session()->get('periode')['tahun'];
		$bulan = substr($periode,0,2);
		$tahun = substr($periode,3,4);
	...
	public function jasperPoSisaReport(Request $request) 
	{
		$file 	= 'sisah';
		...
		{
			return view('oreport_po.sisa')->with(['hasil' => $query]);
		}
	...
	public function jasperPoKartu(Request $request) 
	{
		...
		$query = DB::SELECT("SELECT NO_PO AS NO_BUKTI, DATE_FORMAT(po.TGL,'%d/%m/%Y') as TGL, NAMAS, NA_BRG,
									 KG, KIRIM, SISA, HARGA, 
									TOTAL, NOTES  
								from po where PER='$periode' $sup
							");
		...
		if($request->has('filter'))
		{
			return view('oreport_po.kartu')->with(['hasil' => $query]);
		}
