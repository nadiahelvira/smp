      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-12">
            <div class="card">
              <div class="card-body">
              <form id="entri" target="_" action="{{url('kartuso/posting')}}">
                @csrf	  
                ...
                <!-- <div class="form-group row">
                    ...
                    <div class="col-md-2">						
                      ...
                      <input type="text" class="form-control kodes" id="kodes" name="kodes" placeholder="Pilih Customer" value="{{ session()->get('filter_kodes1') }}" readonly>
                    </div>  
                    <div class="col-md-3">
	  	...
	  	<div class="modal-dialog modal-xl" role="document">
			...
			<div class="modal-body">
				<ul class="nav nav-tabs" id="tabContent">
					<li class="nav-item active"><a class="nav-link active" href="#jual" 		data-toggle="tab">PEMBELIAN / RETUR</a></li>
				...
				<div class="tab-content">
					<div class="tab-pane active" id="jual">
...
<script>
 ...
 //         'scrollX': true,
      ...
      var dTableBCustomer;
		...
		dTableBCustomer = $("#table-bsuplier").DataTable({
...
/////////////////////////////////////////////////////////////////////////////
    	...
    	var dTableBJual;
		loadDataBJual = function(idso){
		
			$.ajax(
			{
				type: 'GET', 	
				url: "{{url('jual/browsekartu')}}",
				data: {
					'IDSO': idso,
				},
				...
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBJual.row.add([
							'<a href="javascript:void(0);" onclick="chooseJual(\''+resp[i].NO_BUKTI+'\', \''+resp[i].NO_SO+'\', \''+resp[i].TGL+'\', \''+resp[i].NAMAC+'\', \''+resp[i].NA_BRG+'\', \''+resp[i].TRUCK+'\', \''+resp[i].KG+'\', \''+resp[i].HARGA+'\', \''+resp[i].TOTAL+'\')">'+resp[i].NO_BUKTI+'</a>',
							resp[i].NO_SO,
							resp[i].TGL,
							resp[i].NAMAC,
							resp[i].NA_BRG,
...
///////////////////////////////////////////////////////////////////////////
		...
		var dTableBPiu;
		loadDataBPiu = function(idso){
		
			$.ajax(
			{
				type: 'GET', 		
				url: "{{url('piu/browsekartu')}}",
				data: {
					'IDSO': idso,
				},
				success: function( response )
				{
					...
					for(i=0; i<resp.length; i++){
						...
						dTableBPiu.row.add([
							'<a href="javascript:void(0);" onclick="choosePiu(\''+resp[i].NO_BUKTI+'\', \''+resp[i].NO_SO+'\', \''+resp[i].TGL+'\', \''+resp[i].KODEC+'\', \''+resp[i].NAMAC+'\', \''+resp[i].URAIAN+'\', \''+resp[i].BAYAR+'\')">'+resp[i].NO_BUKTI+'</a>',
							resp[i].NO_SO,
							resp[i].TGL,
							resp[i].KODEC,
							resp[i].NAMAC,
		...
		dTableBPiu = $("#table-bpiu").DataTable({
...
//////////////////////////////////////////////////////////////////////
		...
		var dTableBKartu;
		loadDataBKartu = function(idso){
		
			$.ajax(
			{
				type: 'GET', 		
				url: "{{url('jual/browsekartu2')}}",
				data: {
					'IDSO': idso,
				},
				success: function( response )
  ...
  function hitung() {
	...
	function simpan() {
  	...
  	cekdetailsx = function(iddetailsx){
		loadDataBJual(iddetailsx);
		loadDataBPiu(iddetailsx);
